...
If a run stops after a partial pass, the checkpoint remains in the lock. Resume the same run while its lease is active. If the lease has expired, use `mole inbox override-stale --run-id <new-run-id> --reason "Confirmed the prior worker stopped"` only after checking the synced folder; the replacement ID must not already have a completion receipt. If the lock is missing, use `--override-missing-lock --run-id <run-id> --reason "..."` only after checking the run history. Every override records the actor, host, time, reason, replacement run, and replaced lock under `governance/run-receipts/inbox-processing/overrides/`.
