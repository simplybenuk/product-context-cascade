  ...
  while (true) {
    try {
      ...
      try {
        ...
        if (Date.now() - stat.mtimeMs > SOURCE_REGISTRY_LOCK_STALE_MS) {
...
function releaseSourceRegistryLock(lockFile) {
  try {
    fs.unlinkSync(lockFile);
  } catch (error) {
...
function withSourceRegistryLock(instanceRoot, callback, options = {}) {
  ...
  const lockFile = acquireSourceRegistryLock(instanceRoot, options);
  try {
    return callback();
  } finally {
    releaseSourceRegistryLock(lockFile);
  }
}
...
function attachmentRecords(instanceRoot, parent, registry, options = {}) {
  ...
  for (const attachment of parent.attachments || []) {
    ...
    const child = makeRecord(instanceRoot, absolute, {
      sourceId,
      contentHash: hashSourceFile(absolute),
