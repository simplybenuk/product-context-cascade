...
export function createStarterSourceRegistry(instanceRoot, options = {}) {
  const file = registryPath(instanceRoot);
  if (fs.existsSync(file)) return loadSourceRegistry(instanceRoot);
  return saveSourceRegistry(instanceRoot, emptyRegistry(options.now ? options.now.toISOString() : null), {
    updatedAt: options.now ? options.now.toISOString() : null
  });
}
...
export function hashSourceFile(filePath) {
  return hashSourceContent(fs.readFileSync(filePath, 'utf8'));
}
...
function makeRecord(instanceRoot, filePath, options = {}) {
  ...
  const contentHash = options.contentHash || options.content_hash || hashSourceContent(content);
...
function attachmentRecords(instanceRoot, parent, registry, options = {}) {
  ...
  for (const attachment of parent.attachments || []) {
    ...
    const sourceId = attachment.source_id || frontmatter.source_id || createSourceId();
    const existing = findRecordById(registry.records, sourceId);
    if (existing) {
...
export function registerSourceFile(instanceRoot, filePath, options = {}) {
  ...
  const conflicts = findSourceConflictsFromRegistry(registry);
  saveSourceRegistry(root, registry, { updatedAt: observedAt });
...
export function registerSource(instanceRoot, recordOrFilePath, options = {}) {
  if (typeof recordOrFilePath === 'string') return registerSourceFile(instanceRoot, recordOrFilePath, options);
  const root = path.resolve(instanceRoot);
  const registry = readRegistryFile(root);
  const record = {
    ...recordOrFilePath,
  ...
  registry.records.push(record);
  saveSourceRegistry(root, registry, { updatedAt: options.updatedAt });
  return { ok: true, created: true, record, conflicts: findSourceConflictsFromRegistry(registry) };
}
...
function workspaceFiles(instanceRoot, options = {}) {
  ...
  return files.filter((file) => path.relative(root, file) !== SOURCE_REGISTRY_REL_PATH);
}
...
export function updateSourcePath(instanceRoot, sourceId, newPath, options = {}) {
  ...
  saveSourceRegistry(root, registry, { updatedAt: options.observedAt });
  return record;
}
...
export function syncSourceRecord(instanceRoot, sourceId, options = {}) {
  ...
  const record = findRecordById(registry.records, sourceId);
  if (!record) throw new Error(`Unknown source ID: ${sourceId}`);
  const result = resolveSource(root, sourceId, { persist: false });
...
export function resolveSource(instanceRoot, sourceId, options = {}) {
  ...
  if (options.persist !== false) saveSourceRegistry(root, registry, { updatedAt: options.observedAt });
  return {
    status: 'resolved',
    source_id: sourceId,
    path: candidate.path,
