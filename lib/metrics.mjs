...
import { inspectInboxProcessing } from './inbox-processing.mjs';
...
export function backfillProcessedInboxMetrics(instanceRoot, options = {}) {
  ...
  const conflictedPaths = new Set((processingState.processedPathConflicts || [])
    .map((item) => item.path));
  const conflictReceiptPaths = new Set((processingState.conflictReceipts || [])
    .map((item) => item.path));
  ...
  for (const receiptFile of receiptFiles) {
    ...
    const receiptRelativePath = toPortablePath(path.join(INBOX_RECEIPTS_REL_DIR, receiptFile));
    if (conflictReceiptPaths.has(receiptRelativePath)) {
      result.receipts_skipped += 1;
      result.conflict_receipts_skipped += 1;
      continue;
    }
    ...
    const processed = normalizeProcessedPaths(receipt.processed, instanceRoot)
      .filter((processedPath) => !conflictedPaths.has(processedPath));
