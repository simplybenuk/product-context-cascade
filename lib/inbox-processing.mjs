...
function readOverrideRecords(instanceRoot, overridesDir) {
  if (!fs.existsSync(overridesDir)) {
    return { overrides: [], invalidOverrides: [], incompleteOverrides: [] };
  }
  const overrides = [];
...
function validateProcessingState(state, options = {}) {
  ...
  if (state.incompleteOverrides?.length && !options.allowIncompleteOverride) {
    return {
      ok: false,
...
function pendingOverrideForReceipt(state, receipt) {
  const overrideId = receipt?.override?.override_id;
  if (!overrideId) return null;
  return (state.incompleteOverrides || [])
    .find((item) => item.override.override_id === overrideId) || null;
}
...
function checkpointInboxProcessingUnlocked(instanceRoot, options = {}) {
  ...
  const alreadyProcessed = newPaths.filter((item) => previouslyProcessed.has(item));
  if (alreadyProcessed.length) {
    return resultForFailure(
      'ALREADY_PROCESSED',
...
function completeInboxProcessingUnlocked(instanceRoot, options = {}) {
  ...
  const canFinalizePendingOverride = Boolean(
    existingReceiptForValidation && pendingOverrideForReceipt(state, existingReceiptForValidation.receipt)
  );
  const validState = validateProcessingState(state, {
    ...options,
    instanceRoot: root,
    allowIncompleteOverride: canFinalizePendingOverride
  });
  if (!validState.ok) return { ...validState, lockPath: paths.lockRelPath };
  ...
  if (requestedRunId) {
    ...
    if (existingReceipt) {
      ...
      const pendingOverride = pendingOverrideForReceipt(state, existingReceipt.receipt);
  ...
  return {
    ...
    message: 'Inbox processing receipt written to ' + written.receiptPath + '.' + finalizationWarning
  };
}
