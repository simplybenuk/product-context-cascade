...
export function looksLikeSyncConflictName(name) {
  ...
  return /(?:\bconflict(?:ed)?[\s_-]+copy\b|\bcopy[\s_-]+of\b|\bsync[\s_-]+conflict\b|\(\s*conflict(?:ed)?[\s_-]+copy\s*\)|\bduplicate[\s_-]+(?:copy|conflict)\b)/.test(text);
}
...
export function heartbeatInboxProcessing(instanceRoot, options = {}) {
  ...
  if (!current.value || !sameLockState(current.value, owned.lock)) {
    return resultForFailure(
      'LOCK_CHANGED',
...
export function overrideStaleInboxProcessing(instanceRoot, options = {}) {
  ...
  const finalCurrent = readJsonIfExists(paths.lockPath);
  if (!finalCurrent.value || !sameLockState(finalCurrent.value, state.lock)) {
    return resultForFailure(
      'LOCK_CHANGED',
      'The stale lock changed while the override was being written. No replacement lock was installed.',
      paths,
      { lock: finalCurrent.value || null, override: audit.record }
    );
  }
