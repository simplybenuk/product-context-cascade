...
export function looksLikeSyncConflictName(name) {
  ...
  return /\b(?:conflict(?:ed)?[\s_-]+copy|duplicate|copy[\s_-]+of|sync[\s_-]+conflict)\b/.test(text);
}
...
function readOverrideRecords(instanceRoot, overridesDir) {
  ...
  for (const entry of fs.readdirSync(overridesDir, { withFileTypes: true })) {
    ...
    if (!parsed.error && parsed.value && typeof parsed.value === 'object') {
      overrides.push({
        path: relativePath(instanceRoot, absolute),
        override: parsed.value
      });
    }
  }
  return overrides.sort((left, right) => left.path.localeCompare(right.path));
}
...
export function inspectInboxProcessing(instanceRoot) {
  ...
  return {
    ...
    overrides: readOverrideRecords(root, paths.overridesDir)
  };
}

function getReceiptForRun(state, runId) {
  const matches = state.receipts.filter((record) => record.run_id === runId);
...
function conflictMessage(state, inboxConflictPaths = []) {
  ...
  if (!conflicts.length && !state.duplicateReceipts.length) return '';
...
function validateProcessingState(state, options = {}) {
  ...
  if (state.lockError || state.lockValidationError) {
    return {
      ok: false,
      code: 'INVALID_LOCK',
...
function verifyOwnedActiveLock(instanceRoot, options = {}) {
  ...
  const requestedRunId = options.runId || options.lockId;
  const lockRunId = getRunId(lock);
  if (requestedRunId && requestedRunId !== lockRunId) {
    return {
      ...resultForFailure(
        'RUN_ID_MISMATCH',
...
function createOverrideRecord(instanceRoot, options, details = {}) {
  ...
  const file = path.join(paths.overridesDir, overrideId + '.json');
  try {
    writeJsonExclusive(file, record);
  } catch (err) {
    if (err.code !== 'EEXIST') throw err;
    const existing = readJsonIfExists(file);
    if (existing.value && JSON.stringify(existing.value) === JSON.stringify(record)) {
      return { record, path: file };
    }
    throw new Error('Override record ' + overrideId + ' already exists with different contents.');
  }
...
export function claimInboxProcessing(instanceRoot, options = {}) {
  ...
  const existingReceipt = requestedRunId ? getReceiptForRun(state, requestedRunId) : null;
...
export function heartbeatInboxProcessing(instanceRoot, options = {}) {
  ...
  const current = readJsonIfExists(owned.paths.lockPath);
  if (!current.value || !sameLockIdentity(current.value, owned.lock)) {
    return resultForFailure(
      'LOCK_CHANGED',
      'The inbox lock changed while the heartbeat was being prepared. Refusing to overwrite or select between lock versions.',
      owned.paths,
      { lock: current.value || null }
    );
  }

  writeJsonAtomic(owned.paths.lockPath, updated);
  return {
    ok: true,
...
export function checkpointInboxProcessing(instanceRoot, options = {}) {
  ...
  if (!current.value || !sameLockIdentity(current.value, owned.lock)) {
    return resultForFailure(
      'LOCK_CHANGED',
      'The inbox lock changed while the checkpoint was being prepared. Refusing to overwrite or select between lock versions.',
      owned.paths,
      { lock: current.value || null }
    );
  }
...
export function overrideStaleInboxProcessing(instanceRoot, options = {}) {
  ...
  const replacementRunId = normalizeRunId(options.runId || options.lockId);
  const current = readJsonIfExists(paths.lockPath);
  if (!current.value || !sameLockIdentity(current.value, state.lock)) {
    return resultForFailure(
      'LOCK_CHANGED',
  ...
  const replacement = buildLease(root, {
    ...
    claimedPaths: options.claimedPaths || state.lock.claimed_paths || []
  }, now, {
    overrideId: audit.record.override_id,
    resumedFromRunId: getRunId(state.lock),
    processedPaths: state.lock.processed_paths || [],
    startedAt: now
  });
...
function createMissingLockClaim(instanceRoot, options, now) {
  const paths = getInboxProcessingPaths(instanceRoot);
  const runId = normalizeRunId(options.runId || options.lockId);
  const audit = createOverrideRecord(instanceRoot, options, {
    type: 'missing-lock',
...
function writeOrReadReceipt(paths, receipt) {
  const safeRunId = getRunId(receipt).replace(/[^A-Za-z0-9._-]/g, '_') || 'run';
  const receiptFile = safeRunId + '.json';
...
function releaseOwnedLock(paths, lock) {
  ...
  if (!sameLockIdentity(current.value, lock)) return { released: false, changed: true };
...
export function completeInboxProcessing(instanceRoot, options = {}) {
  ...
  if (requestedRunId) {
    ...
    if (existingReceipt) {
      ...
      if (lock) {
        if (!sameLockIdentity(lock, existingReceipt.receipt)) {
          return resultForFailure(
            'LOCK_CHANGED',
        ...
        const release = releaseOwnedLock(paths, existingReceipt.receipt);
        if (release.changed) {
          return resultForFailure(
            'LOCK_CHANGED',
            'Receipt for run ' + requestedRunId
  ...
  if (!lock) {
    ...
    const created = createMissingLockClaim(root, options, now);
    lock = created.claim;
