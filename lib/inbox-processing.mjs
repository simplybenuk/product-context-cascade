...
export function looksLikeSyncConflictName(name) {
  ...
  return /(?:\bconflict(?:ed)?[\s_-]+copy\b|\bcopy[\s_-]+of\b|\bsync[\s_-]+conflict\b|\(\s*(?:conflict(?:ed)?|duplicate)(?:[\s_-]+copy)?\s*\)|\bduplicate[\s_-]+(?:copy|conflict)\b)/.test(text);
}
  ...
  if (requestedRunId) {
    ...
    if (existingReceipt) {
      ...
      if (lock) {
        ...
        if (!sameLockIdentity(lock, expectedLock)) {
          return resultForFailure(
            'LOCK_CHANGED',
