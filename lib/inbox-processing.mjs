...
function normalizePathList(instanceRoot, values = []) {
  ...
  if (entries.some((item) => typeof item !== 'string' || !item.trim())) {
    throw new Error('Inbox paths must be nonempty strings.');
  }
...
function validateLockMetadata(lock) {
  ...
  for (const field of ['claimed_paths', 'processed_paths', 'unresolved_paths']) {
    if (lock[field].some((item) => typeof item !== 'string' || !item
      || item !== item.trim() || item.includes('\\') || item.includes('\0')
      || path.posix.isAbsolute(item) || path.posix.normalize(item) !== item
      || item === '.')) {
      return 'Inbox processing lock ' + field + ' must contain canonical nonempty relative string paths.';
    }
  }
  return null;
}
...
function validateReceiptMetadata(receipt) {
  ...
  if ((receipt.processed || []).some((item) => typeof item !== 'string' || !item.trim())) {
    return 'Receipt processed paths must be nonempty strings.';
  }
  return null;
}
...
export function getCountableInboxReceipts(state, instanceRoot) {
  const conflictedFiles = new Set(state.conflictReceipts.map((item) => item.path));
  const conflictedPaths = new Set(state.processedPathConflicts.map((item) => item.path));
  return state.receipts.filter((item) => !conflictedFiles.has(item.path)).map((item) => ({
    ...item,
    receipt: {
      ...item.receipt,
      processed: normalizePathList(instanceRoot, item.receipt.processed || [])
        .filter((value) => !conflictedPaths.has(value))
    }
  }));
}
...
function findProcessedPathConflicts(receipts, instanceRoot) {
  ...
  for (const [runId, runReceipts] of receiptsByRun.entries()) {
    ...
    for (const record of runReceipts) {
      for (const value of record.receipt.processed || []) {
        const canonical = canonicalizeInboxPath(instanceRoot, value);
        if (canonical) union.add(canonical);
      }
    }
...
function buildLease(instanceRoot, options, now, overrides = {}) {
  ...
  const error = validateLockMetadata(lease);
...
function replaceLockIfUnchanged(paths, expected, replacement, operation) {
  const validationError = validateLockMetadata(replacement);
  if (validationError) {
...
function finalizeOverrideRecord(audit) {
...
function overrideStaleInboxProcessingUnlocked(instanceRoot, options = {}) {
  ...
  let finalized;
  try {
    finalized = finalizeOverrideRecord(audit);
  } catch (err) {
    return {
      ok: true,
...
function completeInboxProcessingUnlocked(instanceRoot, options = {}) {
  ...
  if (requestedRunId) {
    ...
    if (existingReceipt) {
      ...
      if (pendingOverride) {
        try {
          finalizeOverrideRecord(pendingOverride);
        } catch (err) {
      ...
      return {
        ...
        receiptPath: existingReceipt.path,
        lockPath: paths.lockRelPath,
        ...(reconciliationWarning ? { warning: reconciliationWarning.trim() } : {}),
  ...
  if (pendingOverride) {
    try {
      finalizedOverride = finalizeOverrideRecord(pendingOverride).record;
    } catch (err) {
